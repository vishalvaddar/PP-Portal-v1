...
const InterviewModel = {
    ...
    try {
        ...
        for (const applicantId of applicantIds) {
            ...
            if (updateRes.rowCount > 0) {
                ...
                results.push({ applicantId, status: 'Skipped', reason: 'Reassignment conditions not met (e.g., same interviewer has assigned can not assign again).' });
            }
        }
  ...
  async getStudentsByInterviewer(interviewerName, nmmsYear) {
    try {
      const { rows } = await pool.query(
        `
        SELECT a.student_name, a.applicant_id,s.interview_round
